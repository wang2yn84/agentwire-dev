...
export interface PortalMessageState {
  agents: number[]
...
export function usePortalMessages(
  ...
  useEffect(() => {
    ...
    const handler = (e: MessageEvent) => {
      ...
      if (e.data?.source !== 'portal') return
      ...
      if (msg.type === 'layoutLoaded') {
        ...
        if (status === 'waiting') {
          ...
          // No notification sound — portal has its own audio system
        }
        ...
        console.log(`[Office] Received ${characters.length} pre-colored character sprites`)
        setCharacterTemplates(characters)
      } else if (msg.type === 'floorTilesLoaded') {
        const sprites = msg.sprites as string[][][]
        console.log(`[Office] Received ${sprites.length} floor tile patterns`)
        setFloorSprites(sprites)
      } else if (msg.type === 'wallTilesLoaded') {
        const sprites = msg.sprites as string[][][]
        console.log(`[Office] Received ${sprites.length} wall tile sprites`)
        setWallSprites(sprites)
      } else if (msg.type === 'workspaceFolders') {
        const folders = msg.folders as WorkspaceFolder[]
        setWorkspaceFolders(folders)
      } else if (msg.type === 'settingsLoaded') {
        // No-op — portal doesn't need VS Code settings
      } else if (msg.type === 'furnitureAssetsLoaded') {
        try {
          const catalog = msg.catalog as FurnitureAsset[]
          const sprites = msg.sprites as Record<string, string[][]>
          console.log(`[Office] Loaded ${catalog.length} furniture assets`)
          buildDynamicCatalog({ catalog, sprites })
          setLoadedAssets({ catalog, sprites })
        } catch (err) {
          console.error(`[Office] Error processing furnitureAssetsLoaded:`, err)
        }
      }
    }
